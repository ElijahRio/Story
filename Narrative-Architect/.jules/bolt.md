...
## 2024-06-25 - Prevent Array Allocation in Network Graph Hook
**Learning:** Using `entities.forEach()` to compute text-based links in `useEffect` created unnecessary closure scopes, slowing down the hot path. Converting it to a standard `for` loop eliminated this overhead.
**Action:** Replaced `.forEach` and chained `.map()` in large performance-critical blocks with pre-allocated standard `for` loops.
